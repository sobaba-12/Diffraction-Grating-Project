...
def CurveFitting(sin_axis, vertical0, horizontal0, diagonal1, diagonal2, wave_number):
    ...
    # This will be used for a for loop to have efficient code
    test = [(I_strip, vertical0), (I_strip, horizontal0),
            (I_diag, vertical0), (I_diag, horizontal0),
            (I_strip, diagonal1), (I_strip, diagonal2),
            (I_circle, vertical0), (I_circle, horizontal0)]

    for val in test:
        popt, pcov = curve_fit(val[0], sin_axis, val[1],
                               p0=[np.max(val[1]), 2e-5])
        param.append(popt)
        perr = np.sqrt(np.diag(pcov))
        I0.append(popt[0])
        error_I0.append(perr[0])
        dim.append(popt[1])
        error_dim.append(perr[1])
        chi.append(ChiSquared(vertical0, I_strip(f, *popt)))

    # Plot for vertical strips
    fig2, ax2 = plt.subplots()
    ax2.plot(sin_axis,vertical0, '.', label="data")
    ax2.plot(f, I_strip(f, *param[0]), label="square/rectangle*")
    ax2.plot(f, I_diag(f, *param[2]), label="diamond")
    ax2.plot(f, I_circle(f, *param[7]), label="circle")
    ax2.legend()
    ...
    ax3.plot(f, I_strip(f, *param[1]), label="square/rectangle*")
    ax3.plot(f, I_diag(f, *param[3]), label="diamond")
    ax3.plot(f, I_circle(f, *param[6]), label="circle")
    ax3.legend()
